...
class HXmlParser
{
	...
	static var line : UInt;
	static var newline : Array<{c:Int, p:Int}>;
	...
	**/
	...
	static function doParse( str : String, strict : Bool, p : Int = 0, ?parent : HXml ) : Int
	{
		var xml:HXml = null;
		var state = S.BEGIN;
		var next = S.BEGIN;
		...
		inline function addChild(xml:HXml) 
		{
			parent.addChild(xml);
			nsubs++;
		}
		while ( !StringTools.isEof( c ) )
		{
			if ( c == 10 || c == 13 )
			{
				if ( newline.length == 0  || newline[ newline.length - 1 ].p != p )
				{
					newline.push( { c:c, p:p } );
				}
			}
			
			if ( newline.length == 2 && newline[0].c == 13 && newline[1].c == 10 )
			{
				line++;
				newline = [];
			}
			...
			switch(state)
			{
				...
				case S.PCDATA:
					if (c == '<'.code)
					{
						...
						var child = HXml.createPCData(buf.toString());
						buf = new StringBuf();
						addChild(child);
						state = S.IGNORE_SPACES;
						next = S.BEGIN_NODE;
				...
				case S.CDATA:
					...
					{
						var child = HXml.createCData(str.substr(start, p - start));
						addChild(child);
						p += 2;
						state = S.BEGIN;
					}
				...
				case S.TAG_NAME:
					if (!isValidChar(c))
					{
						if( p == start )
							throw("Expected node name");
						xml = HXml.createElement(str.substr(start, p - start));
						xml.line = line;
						addChild(xml);
						state = S.IGNORE_SPACES;
						next = S.BODY;
				...
				case S.WAIT_END_RET:
					switch(c)
					{
						case '>'.code:
							if( nsubs == 0 )
								parent.addChild(HXml.createPCData(""));
							return p;
						default :
							throw("Expected >");
					}
				...
				case S.COMMENT:
					if (c == '-'.code && str.fastCodeAt(p +1) == '-'.code && str.fastCodeAt(p + 2) == '>'.code)
					{
						addChild(HXml.createComment(str.substr(start, p - start)));
						p += 2;
						state = S.BEGIN;
					}
				case S.DOCTYPE:
					...
					{
						addChild(HXml.createDocType(str.substr(start, p - start)));
						state = S.BEGIN;
					}
				case S.HEADER:
					if (c == '?'.code && str.fastCodeAt(p + 1) == '>'.code)
					{
						...
						addChild(HXml.createProcessingInstruction(str));
						state = S.BEGIN;
					}
				case S.ESCAPE:
					if (c == ';'.code)
		...
		if (state == S.PCDATA)
		{
			if (p != start || nsubs == 0) {
				buf.addSub(str, start, p-start);
				addChild(HXml.createPCData(buf.toString()));
			}
			return p;
		}
